...
ecoda_object <- create_ecoda_object_helper(
  ...
  metadata = example_data$Zhang$metadata, top_n_hvcs = 3
)
...
```{r}
df <- ecoda_object@freq[, ecoda_object@hvcs[1:3]]
df$group <- ecoda_object@metadata$Tissue

library(ggplot2)
# Convert compositions (A,B,C with A+B+C=1) to triangle coordinates
to_cartesian <- function(A,B,C) {
  x <- 0.5 * (2*B + C) / (A+B+C)
  y <- sqrt(3)/2 * C / (A+B+C)
  data.frame(x, y)
}
...
xy <- to_cartesian(df[,1], df[,2], df[,3])
df <- cbind(df, xy)
ggplot(df, aes(x, y, color=group)) +
  ...
  coord_equal() +
  theme_minimal() +
  labs(title="Simplex plot (pure ggplot2)")
```
