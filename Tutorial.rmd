...
reason, zeros are imputed by adding a pseudocount of + (2/3) (to not distort
ratios) or 2/3 times the smallest value [@martín-fernández2003] in case the
ECODA object was created from a data frame of relative abundances.
...
```{r message=FALSE, warning=FALSE, eval = requireNamespace("SeuratData", quietly = TRUE)}
...
```{r, eval = requireNamespace("SeuratData", quietly = TRUE)}
n_samples <- 40
n_groups <- 2
