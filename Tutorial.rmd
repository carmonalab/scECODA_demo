...
## How can I subset or drop specific cell types?

You may want to exclude cell types that introduce technical noise rather than
biological signal. Common candidates for removal include:

-   Technical artifacts: Red blood cells or platelets from tissue contamination.
-   High-variance types: Fragile cells like neutrophils, which often show
    inconsistent recovery rates across samples.

Removing these "nuisance" populations helps mitigate batch effects and ensures
that compositional changes reflect true biology rather than sample preparation
artifacts.

**Recommended Workflow:**

-   **From a Matrix:** Subset your cell count matrix before creating the
    `ecoda_object`.
-   **From Seurat/SCE:** If you want to work with pseudobulk data, please subset
    your Seurat/SCE object first. Alternatively, convert your object directly to
    an `ecoda_object`. Then, you can extract the count matrix, filter out the 
    unwanted cell types, and re-initialize the object for a clean analysis as
    shown below.

```{r eval=FALSE, include=FALSE}
# From a Matrix:
counts <- example_data$Stephenson$cell_counts_highresolution
ct_remove <- c(
  "Platelets",
  "RBC"
)
counts_sub <- counts[, !colnames(counts) %in% ct_remove]
metadata <- example_data$Stephenson$metadata
...
# Create new subsetted ecoda object
